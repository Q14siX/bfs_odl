"""Coordinator for the BfS ODL integration."""
...
class StrahlenschutzDataUpdateCoordinator(DataUpdateCoordinator[dict[str, dict[str, Any]]]):
    ...
    def __init__(self, hass: HomeAssistant, api: StrahlenschutzApiClient, config_entry: ConfigEntry) -> None:
        self.api = api
    ...
    @property
    def assessment_thresholds(self) -> tuple[float, float]:
        low = float(self.config.get(CONF_THRESHOLD_LOW, DEFAULT_THRESHOLD_LOW_USV_H))
        high = float(self.config.get(CONF_THRESHOLD_HIGH, DEFAULT_THRESHOLD_HIGH_USV_H))
        if low >= high:
            return DEFAULT_THRESHOLD_LOW_USV_H, DEFAULT_THRESHOLD_HIGH_USV_H
        return low, high
